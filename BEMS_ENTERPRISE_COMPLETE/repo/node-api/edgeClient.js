...
class EdgeClient {
  ...
  async readPoints(request = {}) {
    return {
      strategy: "cov_first_batch_read_unavailable_without_edge_grpc",
      results: (request.points || []).map((point) => ({
        ...
        units: "",
        status: "offline",
        error: "Edge gRPC client is not enabled.",
        attempts: 0,
        offline: true,
      })),
    };
  ...
  async subscribeCov() {
    return { accepted: false, status: "disabled", message: "Edge gRPC client is not enabled." };
  }
}
...
class GrpcEdgeClient extends EdgeClient {
  ...
  async readPoints(request) {
    try {
      return await this.call("ReadPoints", request);
    } catch (error) {
      return {
        strategy: "cov_first_read_property_multiple_preferred_fallback_failed",
        results: (request.points || []).map((point) => ({
          ...point,
          success: false,
          value: null,
  ...
  async subscribeCov(request) {
    try {
      return await this.call("SubscribeCov", request);
    } catch (error) {
      return { accepted: false, status: "unreachable", message: error.message };
    }
  }
}
